...
`ast/` is a different thing and follows the opposite rule: `ast/*.sql` are
hand-written scripts, a curated tour of the node set, and are meant to be
edited. Their `ast/*.tree` snapshots are regenerated with
`go test ./parser -update` and reviewed in the diff. Snapshots are meyer's
own goldens; they never outrank the accept/reject corpus below.
